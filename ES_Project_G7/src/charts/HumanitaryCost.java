package charts;
	...
	public class  HumanitaryCost extends JFrame {
		private static final long serialVersionUID = 6294689542092367723L;
		private String name1 = null;
		PieDataset dataset;
		private Map<String, Double> mapa = new HashMap<String, Double>();

		public  HumanitaryCost(String title, String sprintname) {
			super(title);
			...
			for (Card card : trelloconnect.listCardsSprint(trelloconnect.getSprint(this.name1))) {
				for (Member m : trello.getCardMembers(card.getId())) {
					if (!mapa.containsKey(m.getUsername())) {
						mapa.put(m.getUsername(), sprint.membergetTimeEstimated(m.getUsername(), m.getId(), this.name1)*20);
		...
		public static void main(String[] args) throws IOException {
			...
			SwingUtilities.invokeLater(() -> {
				 HumanitaryCost example = new  HumanitaryCost("Custo do Sprint","3rd Sprint");
