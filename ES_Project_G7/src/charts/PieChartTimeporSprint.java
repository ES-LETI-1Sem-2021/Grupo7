...
public class PieChartTimeporSprint extends JFrame {
		...
		for (Card card : trelloconnect.listCardsSprint(trelloconnect.getSprint(this.name1))) {
			for (Member m : trello.getCardMembers(card.getId())) {
				if (!mapa.containsKey(m.getUsername())) {
					mapa.put(m.getUsername(), sprint.membergetTimeSpent(m.getUsername(), m.getId(), this.name1));
	...
	public static void main(String[] args) throws IOException {
		...
		SwingUtilities.invokeLater(() -> {
			PieChartTimeporSprint example = new PieChartTimeporSprint("Tempo por Sprint", "3rd Sprint");
