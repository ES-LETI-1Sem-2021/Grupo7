...
public class PieChartEstimatedTimeTotalforAllSprints extends JFrame {
	private static final long serialVersionUID = 6294689542092367723L;
	private Map<String, Double> mapa = new HashMap<String, Double>();

	public PieChartEstimatedTimeTotalforAllSprints(String title) {
		super(title);
	 ...
	 * 
	 */
	private void createDatasetaux(String sprintname) {

		TrelloConnect trelloconnect = MainWindow.getFrame().getTrelloConnect();
		Trello trello = trelloconnect.getTrello();
		Sprint sprint = new Sprint();

		sprint.getTimeSpent(sprintname);
		DefaultPieDataset dataset = new DefaultPieDataset();
		for (Card card : trelloconnect.listCardsSprint(trelloconnect.getSprint(sprintname))) {
			for (Member m : trello.getCardMembers(card.getId())) {
				if (!mapa.containsKey(m.getUsername())) {
					mapa.put(m.getUsername(), sprint.membergetTimeSpent(m.getUsername(), m.getId(), sprintname));
	...
	public static void main(String[] args) throws IOException {
		...
		SwingUtilities.invokeLater(() -> {
			PieChartEstimatedTimeTotalforAllSprints example = new PieChartEstimatedTimeTotalforAllSprints(
					"Tempo estimado por todos os Sprints");
